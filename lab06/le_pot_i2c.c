...
int main(int argc, char **argv){
   ...
   if(argc==2){
     ...
     if(write(file, alertbuf, 1)!=1){   // enviando os 1 bytes do alertbuf
        perror("Falha ao ajustar o limite do alarme pelo potenciometro!\n");
        return 1;
     }
   }
   ...
   int valor, limite, valorLED, tensaoInteira, tensaoDecimal;
   ...
   printf("A tensão do potenciometro é %d.%d V\n", tensaoInteira, tensaoDecimal);
