...
void loop(){                       // Le o valor no potenciometro a cada 1 segundo
  ...
  tensao = 5.f*static_cast<float>(valorPot)/1023.f;
  buffer[0] = valorPot & 0xFF;     // byte menos significativo
  buffer[1] = valorPot >> 8;       // byte mais significativo
  buffer[5] = byte(tensao);     // byte menos significativo
  buffer[6] = byte((tensao - static_cast<float>(buffer[5])) * 100);       // byte mais significativo
...
void funcaoDadoRecebido(int x){    // funcao chamada ao receber um dado
  ...
  if( dadoRecebido == 0x10 && x == 1 ){  // se for alteracao do limite do alarme pelo potenciometro
    buffer[3]  = (uint8_t)(valorPot >> 8); 
    buffer[2]  = (uint8_t)(valorPot & 0xFF); 
